...
#include "wfc_color.hpp"

/**
 * Read an image. Returns nullopt if there was an error.
 */
std::optional<Array2D<WFCColor>> read_image(const std::string& file_path) noexcept {
  int width;
  int height;
  int num_components;
  ...
  Array2D<WFCColor> m = Array2D<WFCColor>(height, width);
 ...
 */
void write_image_png(const std::string& file_path, const Array2D<WFCColor>& m) noexcept {
